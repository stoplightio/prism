...
function translateFromFormDataParameter(parameters, consumes) {
    const finalBody = {
        contents: consumes.map(mediaType => ({
            mediaType,
            schema: {
    ...
    return parameters.reduce((body, parameter) => {
        ...
        (body.contents || []).forEach(content => {
            ...
            if (parameter.required) {
                const requiredIndex = lodash_1.get(content, 'schema.required', []).length;
...
exports.translateFromFormDataParameter = translateFromFormDataParameter;
function buildEncoding(parameter, content) {
    switch (parameter.collectionFormat) {
        case 'csv':
            return {
