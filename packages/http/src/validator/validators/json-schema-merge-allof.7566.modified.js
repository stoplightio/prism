...
/** 
 * Given a reference string (e.g., "#/__bundled__/contactInfo"), returns an
 * object that matches the original structure that included the $ref.
 */
function noOp$refResolver(ref) {
  return { $ref: ref };
}
  ...
  function mergeSchemas(schemas, base, parents) {
    ...
    schemas = schemas.map(schema => {
      if (isPlainObject(schema) && '$ref' in schema) {
        const schemaFragment = options.$refResolver(schema.$ref);
        console.log(`Resolved '${schema.$ref}' to '${schemaFragment}' using ${options.$refResolver.name}.`);
        return schema;
      }
