...
- Prism is now correctly returning `text/plain` when the document does _not_ specify any Content Type for the examples
- Prism is now correctly returning the example according to the `Accept` header for OAS2 documents
- Prism is now correctly returning `404` instead of `500` in case the requested named example does not exist in the document
